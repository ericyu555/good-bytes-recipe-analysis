...
To evaluate fairness, we examined whether our final model performed differently for recipes with **many ingredients** versus recipes with **few ingredients**. We chose this grouping because the number of ingredients is a natural measure of recipe complexity and was central to our earlier hypothesis testing.
...
The difference in RMSE between the two groups was small, indicating that the model performs similarly across recipes of different complexity levels. This suggests that the model does not systematically favor simpler or more complex recipes when making predictions.
